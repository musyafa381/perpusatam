    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
      <div>
        <div class="badge bg-white text-primary fw-bold px-3 py-1 mb-2 rounded-pill fs-2 shadow-sm">
          <i class="ti ti-category me-1"></i> Taksonomi Buku
        </div>
        <h3 class="text-white fw-bold mb-1">Kategori Buku</h3>
        <p class="text-white-50 mb-0">Kelola pengelompokan subjek dan klasifikasi koleksi buku perpustakaan.</p>
      </div>
      <div>
        <a href="<?= base_url('admin/categories/new'); ?>" class="btn btn-light text-primary fw-bold shadow-sm">
        ...
        <tbody>
          ...
          <?php foreach ($categories as $key => $category) : ?>
            <tr>
              ...
              <td>
                <div class="fw-bold text-dark fs-3"><?= esc($category['name']); ?></div>
              </td>
              <td class="text-center">
                <span class="badge badge-subtle-primary fs-2 px-3 py-2">
                  <i class="ti ti-books me-1"></i><?= $bookCountInCategories[$key] ?? 0; ?> Buku
                </span>
              </td>
              <td class="text-center pe-3">
                <div class="d-flex justify-content-center gap-2">
                  <a href="<?= base_url("admin/categories/{$category['id']}/edit"); ?>" class="btn btn-primary btn-sm px-3 rounded-3">
                    <i class="ti ti-edit me-1"></i> Edit
                  </a>
                  <form action="<?= base_url("admin/categories/{$category['id']}"); ?>" method="post">
                    <?= csrf_field(); ?>
                    <input type="hidden" name="_method" value="DELETE">
                    <button type="submit" class="btn btn-outline-danger btn-sm px-3 rounded-3" data-confirm="Apakah Anda yakin ingin menghapus kategori ini?">
