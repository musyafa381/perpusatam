...
<div class="card">
  <div class="card-body">
    <h5 class="card-title fw-semibold mb-4">Form Tambah Anggota Baru</h5>
    ...
    <form action="<?= base_url('admin/members'); ?>" method="post">
      ...
      <div class="mb-4 p-3 bg-light rounded border border-light-subtle">
        <label class="form-label fw-bold d-block text-dark">Pilih Tipe Anggota:</label>
        <?php $selectedType = $oldInput['member_type'] ?? 'siswa'; ?>
      ...
      <div class="mb-4 p-3 bg-light rounded-3 border border-warning-subtle">
        ...
        <div class="input-group">
          ...
          <input type="text" class="form-control border-primary-subtle <?php if ($validation->hasError('uid')) : ?>is-invalid<?php endif ?>" id="uid" name="uid" value="<?= esc($oldInput['uid'] ?? ''); ?>" placeholder="Tembak barcode ID Card fisik siswa/pegawai (NIS/NIP) atau ketik manual..." autofocus required>
      ...
      <div class="row">
        <div class="col-12 col-md-6 mb-3">
          ...
          <input type="text" class="form-control <?php if ($validation->hasError('first_name')) : ?>is-invalid<?php endif ?>" id="first_name" name="first_name" value="<?= $oldInput['first_name'] ?? ''; ?>" placeholder="Masukkan nama depan" required>
          <div class="invalid-feedback">
        ...
        <div class="col-12 col-md-6 mb-3">
          ...
          <input type="text" class="form-control <?php if ($validation->hasError('last_name')) : ?>is-invalid<?php endif ?>" id="last_name" name="last_name" value="<?= $oldInput['last_name'] ?? ''; ?>" placeholder="Opsional">
          <div class="invalid-feedback">
      ...
      <div class="row">
        <div class="col-12 col-md-6 mb-3">
          ...
          <?php $genderVal = $oldInput['gender'] ?? '1'; ?>
          <div class="my-2 <?php if ($validation->hasError('gender')) : ?>is-invalid<?php endif ?>">
            <div class="form-check form-check-inline">
              <input type="radio" class="form-check-input" id="male" name="gender" value="1" <?= ($genderVal == '1' || $genderVal == 'Male') ? 'checked' : ''; ?> required>
              <label class="form-check-label" for="male">Laki-laki</label>
            </div>
            <div class="form-check form-check-inline">
              <input type="radio" class="form-check-input" id="female" name="gender" value="2" <?= ($genderVal == '2' || $genderVal == 'Female') ? 'checked' : ''; ?> required>
      ...
      <div id="siswa_section" class="p-3 border rounded border-primary bg-primary-subtle mb-4">
        ...
        <div class="row">
          <div class="col-12 col-md-6 mb-3">
            ...
            <?php $instVal = $oldInput['institution'] ?? ''; ?>
      ...
      <div class="mb-4 p-3 bg-light rounded-3 border border-primary-subtle">
        ...
        <?php $selectedManualTier = $oldInput['manual_tier'] ?? 'none'; ?>
        <select class="form-select border-primary" id="manual_tier" name="manual_tier">
      ...
      <!-- Seksi Tambahan (Wajib bagi Petugas, Opsional bagi Siswa) -->
      <div id="additional_fields" class="row">

        <div class="col-12 col-md-6 mb-3">
          <label for="email" class="form-label" id="label_email">Email <span class="text-danger d-none" id="req_email">*</span></label>
          <input type="email" class="form-control <?php if ($validation->hasError('email')) : ?>is-invalid<?php endif ?>" id="email" name="email" value="<?= $oldInput['email'] ?? ''; ?>" placeholder="contoh@domain.com">
          <div class="invalid-feedback">
            <?= $validation->getError('email'); ?>
          </div>
        </div>
        <div class="col-12 col-md-6 mb-3">
          <label for="phone" class="form-label" id="label_phone">Nomor Telepon <span class="text-danger d-none" id="req_phone">*</span></label>
          <input type="tel" class="form-control <?php if ($validation->hasError('phone')) : ?>is-invalid<?php endif ?>" id="phone" name="phone" value="<?= $oldInput['phone'] ?? ''; ?>" placeholder="+62812345678">
          <div class="invalid-feedback">
            <?= $validation->getError('phone'); ?>
          </div>
        </div>
        <div class="col-12 col-md-6 mb-3">
          <label for="date_of_birth" class="form-label" id="label_dob">Tanggal Lahir <span class="text-danger d-none" id="req_dob">*</span></label>
        ...
        <div class="col-12 mb-3">
          <label for="address" class="form-label" id="label_address">Alamat <span class="text-danger d-none" id="req_address">*</span></label>
          <textarea class="form-control <?php if ($validation->hasError('address')) : ?>is-invalid<?php endif ?>" id="address" name="address" rows="2"><?= $oldInput['address'] ?? ''; ?></textarea>
          <div class="invalid-feedback">
      ...
      <button type="submit" class="btn btn-primary mt-3 px-4 py-2"><i class="ti ti-device-floppy me-1"></i> Simpan Data Anggota</button>
    </form>
  </div>
</div>

<script>
  const oldClassVal = '<?= esc($oldInput['class_level'] ?? ''); ?>';
  ...
  function updateClassOptions(institution) {
    const classSelect = document.getElementById('class_level');
    classSelect.innerHTML = '<option value="" disabled selected>--Pilih Kelas / Semester--</option>';
    ...
    options.forEach(opt => {
      const isSelected = (opt === oldClassVal) ? 'selected' : '';
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initMemberForm);
  } else {
    initMemberForm();
  }
</script>
