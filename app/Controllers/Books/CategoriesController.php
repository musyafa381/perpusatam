...
class CategoriesController extends ResourceController
{
    ...
    {
        $categoryName = trim($this->request->getVar('category') ?? $this->request->getVar('name') ?? '');
        ...
        if ($this->request->isAJAX() || $this->request->getHeaderLine('X-Requested-With') === 'XMLHttpRequest') {
            ...
            $existing = $this->categoryModel->where('name', $categoryName)->first();
            if ($existing) {
                return $this->response->setJSON([
                    'status' => true,
            ...
            $this->categoryModel->insert(['name' => $categoryName]);
