...
class RacksController extends ResourceController
{
    ...
    {
        $rackName = trim($this->request->getVar('rack') ?? $this->request->getVar('name') ?? '');
        $floor = trim($this->request->getVar('floor') ?? '1');

        if ($this->request->isAJAX() || $this->request->getHeaderLine('X-Requested-With') === 'XMLHttpRequest') {
            if (empty($rackName)) {
                return $this->response->setJSON([
                    'status'  => false,
                    'message' => 'Nama rak tidak boleh kosong.'
                ]);
            }

            $existing = $this->rackModel->where('name', $rackName)->first();
            if ($existing) {
                return $this->response->setJSON([
                    'status' => true,
            ...
            $this->rackModel->insert([
                'name'  => $rackName,
                'floor' => !empty($floor) ? $floor : 1,
            ]);
            $newId = $this->rackModel->insertID();

            return $this->response->setJSON([
                'status' => true,
                'data'   => ['id' => $newId, 'name' => $rackName]
            ]);
        }
